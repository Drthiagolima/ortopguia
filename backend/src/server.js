...
function parseYesNoIntent(text) {
  ...
  const hasNo = /\b(nao|não|negativo|nenhum|nenhuma)\b/.test(source);
...
function buildEmergencyGuidance(patientName) {
  ...
  return [
    `${name}obrigada por me contar. Pelo que você descreveu, seu caso pode precisar de avaliação imediata.`,
    "Minha orientação segura agora é procurar uma emergência ortopédica imediatamente (hospital com ortopedia/trauma).",
    "Se houver piora neurológica súbita (fraqueza progressiva, perda de sensibilidade, incapacidade de mover) ou trauma grave, acione o SAMU (192).",
    "Se mesmo assim você quiser que eu siga com agendamento agora, responda: AGENDAR.",
  ].join("\n");
}
...
function buildRedFlagQuestion(patientName) {
  const name = patientName ? `${patientName}, ` : "";
  return [
    `${name}vou fazer uma triagem ortopédica rápida para te orientar com segurança, tudo bem?`,
    "Você está com algum sinal de alerta ortopédico AGORA?",
    "- trauma com deformidade visível ou suspeita de fratura exposta",
    "- incapacidade de apoiar/mexer o membro após trauma",
    "- perda de força, dormência progressiva ou perda de sensibilidade",
    "- dor lombar com fraqueza importante, alteração urinária ou anestesia em sela",
    "- articulação muito quente, inchada e dolorosa com febre",
...
async function processInboundWhatsappMessage({ from, text, name }) {
  ...
  if (session.stage === "await_triage_redflags") {
    ...
    if (yesNo === null) {
      const retry =
        "Para eu te orientar com segurança, me responde com SIM ou NAO sobre os sinais de alerta que te enviei.";
    ...
    const recommendation =
      session.recommendedMode === "presencial"
        ...
        : "Pelo seu relato, podemos iniciar com teleconsulta com segurança.";
    const reply = [
      "Obrigada por confirmar. Que bom que não há sinais de alerta imediato.",
      recommendation,
      "Agora me diga dia e horário de preferência (exemplo: 03/06 às 14:30).",
    ].join("\n");
  ...
  if (session.stage === "emergency_guidance") {
    ...
    if (!wantsScheduleNow) {
      const reply =
        "Fico com você nessa. Se optar por seguir para emergência agora, te apoio totalmente. Quando quiser retomar comigo, escreva AGENDAR.";
    ...
    await writeWhatsappSessions(sessionsDb);
    const reply =
      "Tudo bem, vou te ajudar com o agendamento agora. Me informe dia e horário de preferência (exemplo: 03/06 às 14:30).";
  ...
  if (session.stage === "await_datetime") {
    ...
    const recommendation =
      session.recommendedMode === "presencial"
        ? "Pela triagem, minha sugestão inicial é consulta presencial."
        : "Pela triagem, minha sugestão inicial é começar por teleconsulta.";
    const reply = [
      recommendation,
      "Você prefere presencial ou teleconsulta? Se quiser, pode responder: seguir sugestao.",
    ].join("\n");
  ...
  if (session.stage === "await_mode") {
    ...
    if (!mode && includesAnyKeyword(lowered, ["sugestao", "sugestão", "seguir sugestao", "seguir sugestão"])) {
