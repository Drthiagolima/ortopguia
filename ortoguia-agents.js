...
(function (global) {
  ...
  const cfg = {
    // Em desenvolvimento local usa localhost; em produção usa a API dedicada.
    baseUrl: isLocalHost ? "http://localhost:8787" : "https://api.ortopguia.com.br",
    authToken: "",
  };
  ...
  async function gerarDocumento(tipo, payload = {}) {
    ...
    const j = await r.json();
    if (Array.isArray(j.outputs) && j.outputs.length) return j.outputs;
